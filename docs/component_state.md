...
- **Legacy KV**: `kv_get`/`kv_put` are legacy runner-host imports. In this runtime they are routed to the state store with a `"{namespace}:{key}"` composite key; they are not the canonical API.
