...
- raise the workspace/template MSRV to Rust 1.88.0 (matching the requirements of wasmtime/cranelift and the generated scaffolds) and pin CI/local smoke to that toolchain so `cargo check --target wasm32-wasip2` succeeds in both dependency modes (PR-COMP-NEW-08).
